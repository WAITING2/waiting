...
public class StudentDaoImpl implements StudentDao {
    ...
    public List<PaperPO> getPaperByStudentNumber(String studentNumber, Integer paperId) {
        ...
        String sql = "select pa.f_student_number,pa.f_teacher_number,pa.id,pa.paper_content,"
                + " pa.paper_status ,pa.comment ,pa.file_name,pa.uu_file_name,pa.posted_time,pa.t_reply_name,uu_t_reply_name"
                + " from tb_paper pa ";
        ...
        try {
            ...
            while (result.next()) {
                ...
                po.setT_reply_name(result.getString("t_reply_name"));
                po.setUu_t_reply_name(result.getString("uu_t_reply_name"));
                res.add(po);
            }
            connection.close();
        } catch (SQLException e) {
    ...
    @Override
    public void updatePaper2(PaperPO po) {

        String sql = "UPDATE tb_paper SET t_reply_name=?,uu_t_reply_name=?,paper_status=?,comment=? WHERE id=? ";
        ...
        try {
            ...
            statement.setString(1, po.getT_reply_name());

            statement.setString(2, po.getUu_t_reply_name());

            statement.setInt(3, po.getPaper_status());

            statement.setString(4, po.getComment());

            statement.setInt(5, po.getId());
...
//				paperVO.setTeacherName(result.getString("teacherName"));
    ...
    public PaperPO getPaperById(Integer paper_id) {
        ...
        String sql = "select * from tb_paper pa where pa.id = ? ";
        Connection connection = null;
        try {
            connection = basicDataSource.getConnection();
            connection.setAutoCommit(true);
