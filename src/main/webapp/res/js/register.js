...
function  warnNumber() {//^\d{n}$
	//
	var nums = document.getElementById("numberts");

    nums.innerHTML = '长度必须为11个数字，注册成功后学号不允许修改!';

    nums.style.color = 'green';

    nums.style.backgroundColor = '#E2F3FE';

    nums.style.border = '1px solid #BED7EE';

}

function  warnT_Number() {//^\d{n}$
    //
    var nums = document.getElementById("t_numberts");

    nums.innerHTML = '长度必须为10个数字，注册成功后工号不允许修改!';
...
function checkname() {
	...
	if (name == null || name == "" || name.length < 3) {
		ts.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;请按要求输入用户名!";
		ts.style.color = "red";
...
function checkNumber() {
    var tname = document.getElementById("studentNumber").value;
    var reg =  /^\d{11}$/;

    var name = tname.replace(/(&nbsp;)|\s|\u00a0/g, '');

    var ts = document.getElementById("numberts");

    if(!reg.test(name)){
        ts.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;请按要求输入学生学号!";
        ts.style.color = "red";
        ts.style.backgroundColor = '#F8F8F8';
        ts.style.border = '1px solid red';
        return false;
...
//						var d = $.parseJSON(data);
                ...
                if (!data.success) {
                    ts.innerHTML = '&nbsp;<img src="/res/image/wrong.gif"></img>&nbsp;该学生已存在!';
                    ts.style.color = "red";
                    ts.style.backgroundColor = '#F8F8F8';
                    ts.style.border = '1px solid red';
                    return false;
    ...
    ts.innerHTML = '&nbsp;<img src="/res/image/icon_ok.gif"></img>';
    ts.style.color = 'green';
    ts.style.backgroundColor = '#F8F8F8';
    ts.style.border = '0';
    return true;
...
function checkT_Number() {
    var tname = document.getElementById("teacherNumber").value;
    var reg =  /^\d{10}$/;

    var name = tname.replace(/(&nbsp;)|\s|\u00a0/g, '');

    var ts = document.getElementById("t_numberts");

    if(!reg.test(name)){
        ts.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;请按要求教师工号!";
        ts.style.color = "red";
        ts.style.backgroundColor = '#F8F8F8';
        ts.style.border = '1px solid red';
        return false;
...
//						var d = $.parseJSON(data);
                // console.log(d.success);
                if (!data.success) {
                    ts.innerHTML = '&nbsp;<img src="/res/image/wrong.gif"></img>&nbsp;该教师已存在!';
                    ts.style.color = "red";
                    ts.style.backgroundColor = '#F8F8F8';
                    ts.style.border = '1px solid red';
                    return false;
...
function checknic() {
	...
	var ns = document.getElementById("nics");
	if (nic == null || nic == "") {
		ns.innerHTML = "&nbsp;<img src='/res/resimage/wrong.gif'></img>&nbsp;请按要求输入昵称!";
		ns.style.color = "red";
			...
			.post(
					...
					function(data) {
						if (data.success == true) {
							ns.innerHTML = '&nbsp;<img src="/res/image/wrong.gif"></img>&nbsp;该昵称已被使用!';
							ns.style.color = "red";
							ns.style.backgroundColor = '#F8F8F8';
							ns.style.border = '1px solid red';
							return false;
	...
	ns.innerHTML = '&nbsp;<img src="/res/image/icon_ok.gif"></img>';
	ns.style.color = 'green';
	ns.style.backgroundColor = '#F8F8F8';
	ns.style.border = '0';
	return true;
...
function checkpass() {
	...
	if (userPass.length < 6 || userPass.length > 16) {
		pts.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;请按要求输入密码!";
		pts.style.color = "red";
		pts.style.backgroundColor = '#F8F8F8';
		pts.style.border = '1px solid red';
		return false;
	}else if(userPass.replace(/(&nbsp;)|\s|\u00a0/g, '')=="" || userPass.replace(/(&nbsp;)|\s|\u00a0/g, '')==null){
		pts.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;密码不能含空格!";
		pts.style.color = "red";
		pts.style.backgroundColor = '#F8F8F8';
		pts.style.border = '1px solid red';
		return false;
	}
	pts.innerHTML = '&nbsp;<img src="res/image/icon_ok.gif"></img>';
	pts.style.color = 'green';
	pts.style.backgroundColor = '#F8F8F8';
	pts.style.border = '0';
	return true;
...
function checkrpass() {
	...
	if (userRPass.length < 6 || userRPass.length > 15) {
		prts.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;请按要求输入密码!";
	...
	prts.innerHTML = '&nbsp;<img src="/res/image/icon_ok.gif"></img>';
	prts.style.color = 'green';
	prts.style.backgroundColor = '#F8F8F8';
	prts.style.border = '0';
	return true;
...
function checkemail() {
	...
	if (!isEmail(userEmail)) {
		ets.innerHTML = "&nbsp;<img src='/res/image/wrong.gif'></img>&nbsp;请注意邮箱格式!";
		ets.style.color = "red";
		ets.style.backgroundColor = '#F8F8F8';
		ets.style.border = '1px solid red';
		return false;
	}
	ets.innerHTML = '&nbsp;<img src="/res/image/icon_ok.gif"></img>';
